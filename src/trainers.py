...
class Trainer:
    ...
    def save_sequence_output(self, sequence_output, layer_num, iter_num, save_path):
        # 确保保存路径存在
        os.makedirs(save_path, exist_ok=True)
        
        # 将输出转换为numpy数组
        output_np = sequence_output.detach().cpu().numpy()
        
        # 保存为npy文件
        filename = f"{layer_num}layer_{iter_num}iter.npy"
        filepath = os.path.join(save_path, filename)
        np.save(filepath, output_np)
    ...
    def iteration(self, epoch, dataloader, train=True):
        ...
        if train:
            ...
            for i, batch in rec_data_iter:
                ...
                recommend_output = self.model.predict(input_ids, user_ids,self.args.all_sequence_output)
                if self.args.all_sequence_output:
                    for layer_num, layer_output in enumerate(recommend_output):
                        self.save_sequence_output(
                            layer_output,
                            layer_num=layer_num,
                            iter_num=i,
                            save_path="./sequence_output/" + self.args.model_type.lower() + "/"
                        )
                    continue
                # recommend_output,_,_ = self.model.predict(input_ids, user_ids)
                recommend_output = recommend_output[:, -1, :]

                # open when frequency spectrum analysis
                # if i == 0:
                #     np.save("./recout/" + self.args.model_type.lower() + "_"+"output_batch.npy", recommend_output.detach().cpu().numpy())
            ...
            if self.args.all_sequence_output:
                self.logger.info("get sequence_output finish，system exit!!!")
                exit(-1)
